...
if __name__ == '__main__':
    # Specify the video path (TODO: Update with something more friendly)
    video_path = os.path.join(os.getcwd(), settings.video_folder, settings.video_file)

    # initialize spherical undistorter (TODO: See how we are going to handle this)
    undistorter = tools.Undistorter('remap', 'gph3+1080-60fps-NARROW.npz')

    # Initialize main tracking objects
    ant = ObjectTracker('ant', 'ColorMatching', (120, 120), 'manual')
    camera = CameraTracker()
    scenario = TrackingScenario([ant], camera, undistorter)

    # Track the video using the preconfigured scenario
    retval, message = scenario.track(video_path)
    print(message)
