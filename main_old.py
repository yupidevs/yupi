...
cap = cv2.VideoCapture(os.path.join(os.getcwd(), settings.video_folder, settings.video_file))
...
U = tools.Undistorter(settings.correction_method, settings.camera_correction_matrix)
...
if __name__ == '__main__':
    ...
    while True:
        ...
        if iteration == 0:
            cap.set(cv2.CAP_PROP_POS_FRAMES, settings.first_frame)
            ret, prev_frame = cap.read()
            
            # correct spherical distortion
            if settings.correct_spherical_distortion:
                prev_frame = U.fix(prev_frame)

            # init ROI coordinates manually by user input
            prev_cXY = (None, None)
            if settings.roi_initialization == 'manual':
        ...
        if settings.roi_initialization == 'auto':
